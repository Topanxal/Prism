...
class JobManager:
    ...
    async def execute_generation_workflow(
        ...
        if settings.mock_mode:
            logger.info("mock_mode_active", job_id="MOCK_JOB")
            
            # Use a fixed template ID for mock
            template_id = "medical_scenes/mock_v1"
            template_version = "1.0.0"
            
            # Get mock data
            mock_ir = MockDataService.get_mock_ir()
            mock_shot_plan = {"shots": MockDataService.get_mock_shot_plan(), "duration_s": 20}
            mock_shot_assets = MockDataService.get_mock_shot_assets()
            
            # Create job record with mock data
            job = JobDB.create_job(
                db=db,
                user_input_redacted=user_input, # Mock redaction
                user_input_hash="mock_hash",
                pii_flags=[],
            ...
            transition_state(db, job.job_id, "SUBMITTED", "mock_submitted")
            transition_state(db, job.job_id, "RUNNING", "mock_running")
            
            # Update assets
            JobDB.update_job_assets(db, job.job_id, mock_shot_assets)
            
            # Transition to SUCCEEDED
            transition_state(db, job.job_id, "SUCCEEDED", "mock_complete")
