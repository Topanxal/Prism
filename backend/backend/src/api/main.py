...
logger.info(f"Mounting static root: {static_root}")

# Explicitly mount /vedios to the videos directory to handle direct access
# This is a fallback/fix for the 404 issue where frontend requests /vedios/mock_video.mp4
# Ensure we are using the absolute path to the data/static/vedios directory
# The _resolve_static_root function ensures settings.static_root is absolute
# On Windows, path joining might be tricky with mounts, so we ensure absolute path
# Use hardcoded path relative to backend root to be absolutely sure
backend_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
video_dir = os.path.join(backend_root, "data", "static", "vedios")
logger.info(f"Mounting video dir (hardcoded): {video_dir} to /vedios")

if not os.path.exists(video_dir):
    os.makedirs(video_dir, exist_ok=True)
# Mount at /vedios
# Using name="static_videos_root"
app.mount("/vedios", StaticFiles(directory=video_dir), name="static_videos_root")
# Also mount under static prefix for correctness
app.mount(f"{settings.static_url_prefix}/{settings.static_video_subdir}", StaticFiles(directory=video_dir), name="static_videos")
